...
export function generateCrystalKing(scene: GameScene, team: 'ally' | 'enemy', props: Partial<CharacterProps> = {}): Character {
    const entity = new Character(scene, props.x || 400, props.y || 300, props.speed || 0.5, props.textureKey || 'crystalKing',
        props.frameWidth || 288, props.frameHeight || 128, props.displayWidth || 288*3, props.displayHeight || 128*3,
        props.uiOffsetY || 0);

    if(props.maxHp) {
        entity.setMaxHP(props.maxHp);
    }
    ...
    entity.onAttackFrame = (attackName: string, frameIndex: number): void => {
        ...
        switch(attackName) {
            case 'attack1':
                applyAttackDamage(scene, entity, enemies, 100, 150, 60, 100, 10);
                break;
            case 'attack2':
                applyAttackDamage(scene, entity, enemies, 60, 150, 120, 100, 20);
                break;
            case 'attack3':
                applyAttackDamage(scene, entity, enemies, 60, 100, 120, 150, 25);
                break;
            case 'attack4':
                applyAttackDamage(scene, entity, enemies, 280, 100, 200, 180, 15);
                break;
            case 'special':
                applyAttackDamage(scene, entity, enemies, 250, 100, 350, 180, 30);
                break;
        }
    }
...
export function generateFireKing(scene: GameScene, team: 'ally' | 'enemy', props: Partial<CharacterProps> = {}): Character {
    const entity = new Character(scene, 
        props.x || 800, props.y || 400, props.speed || 0.5, props.textureKey || 'fireKing', 
    props.frameWidth || 288, props.frameHeight || 128, props.displayWidth || 288*3, props.displayHeight || 128*3,
        props.uiOffsetY || 0);
    ...
    entity.onAttackFrame = (attackName: string, frameIndex: number): void => {
        ...
        switch(attackName) {
            case 'attack1':
                applyAttackDamage(scene, entity, enemies, 140, 180, 150, 60, 10);
                break;
            case 'special':
                applyAttackDamage(scene, entity, enemies, 200, 150, 250, 130, 50);
        }
    }
...
export function generateFrostGuardian(scene: GameScene, team: 'ally' | 'enemy', props: Partial<CharacterProps> = {}): Character {
    const entity = new Character(scene, props.x || 1000, props.y || 200, props.speed || 0.5, 
        props.textureKey || 'frostGuardian', 
        props.frameWidth || 192, props.frameHeight || 128, props.displayWidth || 192*3, props.displayHeight || 128*3, props.uiOffsetY || -140);
    ...
    entity.onAttackFrame = (attackName: string, frameIndex: number): void => {
        const enemies = team === 'ally' ? scene.getEnemies() : scene.getAllies();
        applyAttackDamage(scene, entity, enemies, 150, 20, 250, 100, 10);
    }
...
export default function generateCharacter(scene: GameScene, team: 'ally' | 'enemy', character: AviableCharacter, props: Partial<CharacterProps> = {}): Character {
    switch(character) {
        case 'fireKing':
