...
export default class Character extends Phaser.GameObjects.Container {
  ...
  private attacksDistances: Record<
    CharacterAttackType,
    { x: number; y: number }
  > = {
    special: { x: 0, y: 0 },
  ...
  public getAttacksConfig() {
    return {
      ...
      hasSpecialAttack: this.hasSpecialAttack,
    };
  }
  ...
  public update(time: number, delta: number): void {
    ...
    if (this.movingTarget) {
      ...
      if (dist <= this.movingTarget.stopDistance) {
        ...
        if (step >= remain) {
          const targetX =
            this.movingTarget.x - nx * this.movingTarget.stopDistance;
          const targetY =
            this.movingTarget.y - ny * this.movingTarget.stopDistance;
  ...
  private createHPBar(): void {
    ...
    this.hpText = this.scene.add
      .text(0, offsetY - 15, `${this.currentHP}/${this.maxHP}`, {
        fontSize: "12px",
        color: "#ffffff",
        align: "center",
  ...
  private updateHPBar(): void {
    ...
    // Обновляем текст
    this.hpText.setText(
      `${Math.max(0, Math.floor(this.currentHP))}/${this.maxHP}`,
    );
  }
