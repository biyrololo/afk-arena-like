...
export default function ShopPage() {
  ...
  const [selectedCategory, setSelectedCategory] = useState<'all' | ShopItem['type']>('all');
    ...
    <ResponsiveUI>
      ...
      >
        <div className="absolute inset-0 py-4 flex">
          {/* Back Button */}
          <button 
            className="text-3xl absolute top-4 left-4 text-white cursor-pointer z-10"
            onClick={() => navigate('/')}
          >
            Назад
          </button>
          ...
          <div className="w-full max-w-6xl mx-auto flex flex-col gap-6">
            ...
            <div className="flex justify-center gap-4">
              {(['all', 'character', 'equipment', 'consumable'] as const).map(category => (
                <button
                  key={category}
                  className={`
              ...
              {filteredItems.map(item => {
                ...
                return (
                  ...
                  >
                    {/* Item Header */}
                    <div className="mb-4">
                      <h3 className={`text-2xl font-bold ${rarityClasses.split(' ')[0]} mb-2`}>
                        {item.name}
                      </h3>
            ...
            {filteredItems.length === 0 && (
              <div className="text-center py-12">
                ...
                <p className="text-2xl text-gray-400">Нет товаров в этой категории</p>
              </div>
            )}
          </div>
        </div>
