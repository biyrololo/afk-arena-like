...
import background from '@/assets/backgrounds/lava_castle_bg.webp'
...
export default function GameStartTower() {
  ...
  const [towerFloor, lastSquad, setLastSquad] = usePlayerStore(
    useShallow((state) => [
      state.towerFloor,
      state.lastSquad,
      state.setLastSquad,
  ...
  const enemiesPower = useMemo(() => {
    const stage = generateTowerStage(towerFloor);
    if (!stage) return 0;
      ...
      >
        <div className="absolute inset-0 py-4 flex">
          ...
          <>
            ...
            <p className="text-white text-3xl absolute top-31 left-4 w-[450px] text-shadow-[0px_0px_10px_rgba(0,0,0,0.8)]">
              Это режим бесконечного восхождения. Проходите этаж за этажом, побеждая всё более сильных противников, чтобы получить редкие награды, снаряжение и ресурсы для развития ваших героев.
            </p>
          </>
        </div>
      </div>
